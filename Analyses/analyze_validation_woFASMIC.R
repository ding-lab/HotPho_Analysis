...
validation_all$Sources2 = gsub("MDAnderson,","",validation_all$Sources)
validation_all$Sources2 = gsub(",MDAnderson","",validation_all$Sources2)
validation_all$Sources2 = gsub("MDAnderson","",validation_all$Sources2)
...
validation_all_act_updated = validation_all_act[validation_all_act$Sources2 != "",]
validation_all_act_updated2 = validation_all_act_updated[!(validation_all_act_updated$Sources2 %in% c("ClinVar","DoCM","literature","OncoKb")),]
cat("Number of activating mutations:",nrow(validation_all_act_updated2),"\n")

# functional mutation in clusters
annotated_cluster_score = merge(annotated_cluster,validation_all_act_updated2,by=c("Transcript","Mutation_Gene"),all.x=T)
...
write.table(file = "output/annotated_clusters_w_functional_mutations_woFASMIC.tsv", activating_annotated_cluster_score,quote=F, sep = '\t',row.names = F)
...
write.table(file = "output/functional_mutation_cluster_wKeySites_woFASMIC.tsv", numOfSites,quote=F, sep = '\t',row.names = T)
...
p = p + labs(y= "Count", x= "")
p
outFile = paste("output/coclustered_residues_to_functional_mutation_counts_woFASMIC.pdf")
...
p
outFile = paste("output/coclustered_residues_to_functional_mutation_woFASMIC.pdf")
